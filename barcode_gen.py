...
def generate_qr_code(name, price):
    """Generates a QR code containing name and price, saves it to /tmp."""
    try:
        data = f"Name: {name}, Price: {price}"
        qr = qrcode.make(data)
...
def upload_qr_to_supabase(image_path, name):
    ...
    try:
        ...
        return f"{SUPABASE_URL}/storage/v1/object/public/{SUPABASE_BUCKET}/static/{name}_qr.png"
    except Exception as e:
...
def store_qr_in_db(name, price, qr_url):
    ...
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("INSERT INTO qr_codes (name, price, qr_code_image_path) VALUES (%s, %s, %s)",
                    (name, price, qr_url))
        conn.commit()
        cur.close()
        release_db_connection(conn)
...
def generate_qrcode():
    """API endpoint to generate a QR code and store product details."""
    data = request.json
    name = data.get("name")
...
def get_qr():
    """Fetches QR code details from the database using product name."""
    data = request.json
    name = data.get("name")

    if not name:
        return jsonify({"isSuccess": False, "message": "Product name is required"}), 400
